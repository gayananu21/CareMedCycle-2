...
if (
    ...
    isset($data['therapists']) && is_array($data['therapists'])
    ...
    if ($stmt->execute()) {
       ...
       foreach ($patients as $patient_id) {
        ...
        foreach ($therapists as $therapist_id) {
            $stmt_patient = $conn->prepare("INSERT INTO session_therapist (session_id, therapist_id) VALUES (?, ?)");
            $stmt_patient->bind_param('ii', $session_id, $therapist_id);
            $stmt_patient->execute();
        }
